...
requireAll(__webpack_require__(15));
 var WP_SVG_XHR = new XMLHttpRequest(); WP_SVG_XHR.open('GET', '/etc.clientlibs/zb-common/clientlibs/clientlib-base/resources/img/iconset-6815d201a3041e9406899d326894e088.svg', true); WP_SVG_XHR.onload = function() { if (!WP_SVG_XHR.responseText || WP_SVG_XHR.responseText.substr(0, 4) !== '<svg') { throw Error('Invalid SVG Response'); } if (WP_SVG_XHR.status < 200 || WP_SVG_XHR.status >= 300) { return; } var div = document.createElement('div'); div.innerHTML = WP_SVG_XHR.responseText; document.body.insertBefore(div, document.body.childNodes[0]); }; WP_SVG_XHR.send();
